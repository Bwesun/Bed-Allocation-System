...
$id=$_POST['name'];
$sql="INSERT INTO beds(bed_name)VALUES('$id')";
$result=mysql_query($sql);

//echo $sql;

if($result){
echo "<script>alert('Bed Has been Added Successfully!')</script>";
}
else{
echo "<script>alert('Error: Bed Was not Added! Please Try Again! Ensure the bed name is not the same with others')</script>";
}
?>
<div id="content">
<h3>BEDS:</h3>
  ...
  $sta=0;
  $sql="SELECT * FROM beds ORDER BY bed_id ASC";
  $result=mysql_query($sql);
  $i=1;
  ?>
    <table cellpadding="3">
  ...
  <div align="left">
   ...
   <form action="add_bed_ac.php" method="post">
   
   <input type="text" name="name" placeholder="Enter Bed Space Name" />
